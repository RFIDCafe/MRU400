...
public:
	...
	/// @return #{DOC_LIB_RETURN_VALUE}
    int readDiagnostic(FEDM::FunctionUnit::PdeviceHfPcDiag & diag);

    /// @brief Get the current people counter values
    /// @param[out] values The current people counter values
	/// @return #{DOC_LIB_RETURN_VALUE}
    int getCounterValues(FEDM::FunctionUnit::PdevicePeopleCounterValues & values);

    /// @brief Set the people counter values
    /// @param[in] values The new people counter values
	/// @return #{DOC_LIB_RETURN_VALUE}
    int setCounterValues(const FEDM::FunctionUnit::PdevicePeopleCounterValues & values);
